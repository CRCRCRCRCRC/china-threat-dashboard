...
class MilitaryScraper:
    ...
    def scrape(self) -> Dict[str, Any]:
        ...
        此爬蟲會處理 ASP.NET 的 __doPostBack 機制來進入詳情頁。
        """
        logging.info("正在從國防部網站爬取詳細軍事動態...")
        try:
            session = requests.Session()
            response = session.get(self.url, headers=self.headers, timeout=self.timeout, verify=False)
            response.raise_for_status()
            soup = BeautifulSoup(response.text, 'html.parser')

            links = soup.select('a[href*="__doPostBack"]')
            if not links:
                logging.warning("在國防部網站上找不到動態連結 (PostBack Links)。可能是網站結構已變更。")
                return self._get_fallback_data("找不到動態連結")
            ...
            for i, link in enumerate(links[:5]):
                ...
                details_response = session.post(self.url, headers=self.headers, data=post_data, timeout=self.timeout, verify=False)
...
>>>>>>> 582f439752d7dd09671e0ddbe1ded2923f47c81d
            ...
            if not daily_intrusions:
                logging.warning("無法從任何連結中提取侵擾數據。")
                return self._get_fallback_data("無法提取侵擾數據")

            total_incursions_last_week = sum(daily_intrusions)
            logging.info(f"爬取完成：近五日總擾台 {total_incursions_last_week} 次。最新偵獲共機 {latest_aircrafts} 架次, 共艦 {latest_ships} 艘次。")
...
=======
    ...
    def _get_fallback_data(self, error_msg):
        """生成並回傳備用數據"""
        logging.warning(f"Using fallback military data due to error: {error_msg}")
>>>>>>> 582f439752d7dd09671e0ddbe1ded2923f47c81d
        return {
